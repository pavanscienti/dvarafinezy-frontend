	...
	doAPIRequestWithLoader(API.METHOD_GET, API.PATH_LEAD_FULFILLMENTS ,params, function(json){
		if(json != null && json.code == 200 && json.data){		
			if(json.data.length == 0){
				...
				header += "<th>Pre Approved Product </th>";
				...
				for(var position in json.data){
					...
					row += "<td>" + leadfulfillmentdata.pre_approved_product + "</td>" ;
...
function getBranchManagersList2(branchcode , selectboxid) {
		...
		$('table>tbody .enb_mgr_list').each(
			...
			function (index , selectbox) {
			  if (index == indexval)  {
				...
				getBranchManagersList2(branchcode ,  selectbox.id )

			  }
			  else {
				selectbox.disabled = true ;
				// $('#'+selectbox.id + "> option").html("")
			  }
