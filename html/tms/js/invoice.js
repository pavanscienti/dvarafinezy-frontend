...
function CreateInvoice(){
	$('#invoiceForm').validator().on('submit', function( event ) {	
		...
		let formData = new FormData();
     	formData.append("file", document.getElementById('invoicefile_1').files[0]);

		doAPIRequestWithLoader2(API.METHOD_POST, 
			API.PATH_INVOICE+"?action=uploadVendorTransactions&vendor_id="+vendor+"&year="+year+"&month="+month, 
			formData, 
			function (json) {
			showLoader(false);
